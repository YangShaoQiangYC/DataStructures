...
class Node {
     ...
     *      6）当前节点右节点，指向新节点
     *  3.双旋转
     *   在添加完节点后需要进行左旋转，如果根节点的右子树的（其左子树的高度大于其右子树的高度），则根节点的右子树需要先进行右旋转之后，根节点再进行左旋转
     *   在添加完节点后需要进行右旋转，如果根节点的左子树的（其右子树的高度大于其左子树的高度），则根节点的左子树需要先进行左旋转之后，根节点再进行右旋转
     * @param node 被添加的节点
     */
    public void add(Node node) {
        if (null == node) {
