...
/**
 * 线索化二叉树
 *  1）n个节点的二叉链表中含有n+1个空指针域（公式：2n-(n-1) = n+1）。利用二叉链表中的空指针域，存放指向该节点的某种遍历次序下的前驱和后继节点的指针（这种附加的指针称为“线索”）。
 *  2）这种加了线索的二叉链表称为线索链表，相应的二叉树称为线索二叉树。根据线索性质的不同，线索二叉树可分为前序线索二叉树，中序线索二叉树和后序线索二叉树三种。
 *  3）一个节点的前一个节点，称为前驱节点。
 *  4）一个节点的后一个节点，称为后继节点。
